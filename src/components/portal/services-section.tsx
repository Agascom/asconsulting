        ...
        <div className="mb-14 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {CORE_SERVICES.map((srv) => (
            <button
              ...
              className="group relative flex flex-col justify-between rounded-2xl border border-slate-200 bg-slate-50 p-5 text-left text-slate-800 transition-all duration-300 hover:-translate-y-0.5 hover:border-emerald-300 hover:bg-emerald-50 hover:shadow-md"
            >
              {srv.tag && (
                <span className="mb-3 self-start rounded-full bg-emerald-100 px-2.5 py-0.5 text-[10px] font-bold text-emerald-800">
                  {srv.tag}
                </span>
              )}
              ...
              <div className="space-y-3">
                ...
                <div>
                  <h3 className="font-serif text-base font-bold text-slate-900">{srv.title}</h3>
                  <p className="mt-1 line-clamp-2 text-xs text-slate-500">{srv.shortDesc}</p>
                </div>
              </div>

              <div className="mt-4 flex items-center justify-between border-t border-slate-200 pt-3 text-xs font-semibold">
...
function ServiceDetailBlock({
  ...
  return (
    <div
      ...
      className="relative overflow-hidden rounded-3xl border border-slate-800 bg-slate-900 p-6 text-white shadow-2xl scroll-mt-28 sm:p-8 lg:p-10"
    >
      <div className="grid grid-cols-1 items-start gap-8 lg:grid-cols-12">
        ...
        <div className="space-y-6 lg:col-span-7">
          <div className="flex items-center gap-3">
            <div className="rounded-xl border border-emerald-700 bg-emerald-900 p-3 text-amber-300">
              <ServiceIcon iconName={service.iconName} className="h-7 w-7" />
            </div>
            <div>
              <span className="text-xs font-bold uppercase tracking-widest text-amber-400">
                Service N°{String(index + 1).padStart(2, "0")} — Pilier d'Expertise
              </span>
          ...
          <p className="text-sm leading-relaxed text-slate-300 sm:text-base">{service.fullDesc}</p>
        ...
        <div className="space-y-5 rounded-2xl border border-emerald-800 bg-emerald-950 p-6 lg:col-span-5">
          ...
          <div className="space-y-2">
            <span className="text-[11px] font-semibold text-slate-300">Structures Concernées :</span>
